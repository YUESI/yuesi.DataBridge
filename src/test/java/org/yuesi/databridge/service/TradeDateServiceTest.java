...
class TradeDateServiceTest {
	...
	@Test
	void testCount() {
		System.out.println(tradeDateService.count());
	...
	@Test
	void testQueryByTradeDateBetween() {
		List<TradeDate> result;
		try {
			result = tradeDateService.queryByTradeDate("2020-01-01", "2020-12-31");
	...
	void testIsOpen() {
		try {
			assertTrue(tradeDateService.isOpen("2020-04-10"));
		} catch (ParseException e) {
			e.printStackTrace();
		}
	}
